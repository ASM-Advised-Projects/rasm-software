...
#ifndef RASM2_VISION_POSE_ESTIMATOR_HPP
#define RASM2_VISION_POSE_ESTIMATOR_HPP
...
private:
  cv::VideoCapture cap;
  dlib::frontal_face_detector marker_detector;
  dlib::shape_predictor marker_predictor;
  dlib::frontal_face_detector face_detector;
  dlib::shape_predictor face_predictor;

public:
  PoseEstimator(int camera_index)
  {
    ...
    dlib::deserialize("../test/filesysroot/rasm_2_1/data/vision/prediction/face_model_68_points.dat") >> face_predictor;

    dlib::deserialize("../test/filesysroot/rasm_2_1/data/vision/detection/images/detector.svm") >> marker_detector;
    dlib::deserialize("../test/filesysroot/rasm_2_1/data/vision/detection/images/sp.dat") >> marker_predictor;
  }
  ...
  Pose get_face_pose()
  {
    ...
    if (faces.size() > 0)
    {
      dlib::full_object_detection shape = face_predictor(cimg, faces[0]);
  ...
  Pose get_marker_pose()
  {
    ...
    // Detect faces
    std::vector<dlib::rectangle> faces = marker_detector(cimg);

    if (faces.size() > 0)
    {
      dlib::full_object_detection shape = marker_predictor(cimg, faces[0]);
